...
public final class ObjectHandler {
				...
				//crucible
					...
					case "manhole":
						if (objectDef.containsOption(0, "Open")) {
							...
							//}
						}else if (objectDef.containsOption(0, "Climb-down")) {
					...
					case "door":
						if (object.getType() == 0
						&& (objectDef.containsOption(0, "Open") || objectDef.containsOption(0, "Close") || objectDef
	...
	public static boolean handleDoor(Player player, WorldObject object, long timer) {
		...
		if(object.getDefinitions().getFirstOption().equalsIgnoreCase("open")){
			WorldObject openedDoor = new WorldObject(object.getId()+1,
					object.getType(), object.getRotation() + 1, object.getX(),
					object.getY(), object.getPlane());
			
			if(openedDoor.getDefinitions().getFirstOption().equalsIgnoreCase("open"))
				openedDoor = new WorldObject(object.getId()+2,
						object.getType(), object.getRotation() + 1, object.getX(),
						object.getY(), object.getPlane());
			...
			WorldObject closedDoor = new WorldObject(object.getId() - 2,
					object.getType(), object.getRotation() - 1, object.getX(),
					object.getY(), object.getPlane());		
			
			if(closedDoor.getDefinitions().getFirstOption() == null);
				closedDoor = new WorldObject(object.getId() - 1,
						object.getType(), object.getRotation() - 1, object.getX(),
						object.getY(), object.getPlane());
			...
			if (object.getRotation() == 0)
				...
				closedDoor.moveLocation(0, 0, 0);
			else if (object.getRotation() == 2)
				closedDoor.moveLocation(0, 1, 0);
			else if (object.getRotation() == 3)
				closedDoor.moveLocation(-1, 0, 0);
			
			/*if (World.isSpawnedObject(object)){	// door open and was originally closed*/
				
				World.removeObject(object, false);
				World.spawnObject(closedDoor, false);
			/*} */
	...
	private static boolean handleDoor(Player player, WorldObject object) {
		return handleDoor(player, object, 300000);
	}
	...
	private static boolean handleLadder(Player player, WorldObject object,
		...
		if (option.equalsIgnoreCase("Climb-up")) {
			if (player.getPlane() == 3)
				return false;
			else if(object.getId() == 29355)	// lumbridge cellar
				player.useStairs(828, new WorldTile(player.getX(), player.getY() - 6400, player.getPlane()), 1, 2);
			else
				player.useStairs(828, new WorldTile(player.getX(), player.getY(),
					player.getPlane() + 1), 1, 2);
		} else if (option.equalsIgnoreCase("Climb-down")) {
			if (player.getPlane() == 0){
				if(object.getId() == 36687){ // lumbridge cellar
					player.useStairs(828, new WorldTile(player.getX(), player.getY()+6400, player.getPlane()), 1, 2);
				} else
					return false;
			} else{
